...
const lhs_is_prototype: string = 'or (eq? $kind "p") (eq? $kind "prototype")'
const rhs_is_prototype: string = 'or (eq? &kind "p") (eq? &kind "prototype")'

const lhs_is_function:  string = 'or (eq? $kind "f") (eq? $kind "function")'
const rhs_is_function:  string = 'or (eq? &kind "f") (eq? &kind "function")'

const lhs_is_proto_and_rhs_is_func: string = $'and ({lhs_is_prototype}) ({rhs_is_function})'
const lhs_is_func_and_rhs_is_proto: string = $'and ({lhs_is_function}) ({rhs_is_prototype})'

const lhs_is_file: string = 'or (eq? $kind "F") (eq? $kind "file")'
const rhs_is_file: string = 'or (eq? &kind "F") (eq? &kind "file")'

const lhs_is_header: string = 'or (eq? $kind "h") (eq? $kind "header")'
const rhs_is_header: string = 'or (eq? &kind "h") (eq? &kind "header")'
...
def Build_Module_Tags(project: Project, module: Module): void
	...
	if tag_sort && !!ReadTags_Command
	    var readtags_command_line: list<string> = BuildReadTagsSortCommand(ReadTags_Command + [ '--extension-fields', '--line-number', '--escape-output' ], tags_file_name,
		    '--with-pseudo-tags',
		    '--sorter', $'(if (eq? $name &name) ({sort_condition}) (<> $name &name))',
		    '--list'
		    )
